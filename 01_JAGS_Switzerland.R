...
# Goal: Run JAGS for three models
## M1: Poisson
...
ggplot(data.frame(y_long)) +
  ...
  geom_line(data = df_pois, aes(x = x, y = y, color = 'red'), linetype = 'solid', show.legend = T) +
  geom_line(data = df_negbin, aes(x = x, y = y, color = "green"), linetype = 'longdash', show.legend = T) +
  geom_line(data = df_genpois, aes(x = x, y = y, color = "blue"), linetype = "dotdash", show.legend = T) +
